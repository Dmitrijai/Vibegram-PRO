...
export async function openShorts(
  ...
  if (!skipPushState && window.location.hash !== "#shorts") {
    window.history.pushState(
      { type: "shorts", shortId: initialShortId, authorId: authorFilterId },
      "",
      "#shorts",
    );
  }
...
export function closeShortsSearch() {
  if (window.location.hash === "#shorts-search") {
    ...
    if (
      activeVideo &&
      document.getElementById("shorts-screen")?.classList.contains("hidden") ===
        false
    ) {
...
export async function openShortsSubscriptions() {
  ...
  list.innerHTML =
    '<div class="flex h-full items-center justify-center"><div class="w-8 h-8 border-4 border-blue-500 border-t-transparent rounded-full animate-spin"></div></div>';
  ...
  if (subs.length > 0) {
    try {
      ...
      if (data && data.length > 0) {
        ...
        list.innerHTML =
          '<div class="text-center text-gray-500 mt-10 text-sm">Подписки отсутствуют</div>';
      }
    } catch (e) {
      console.error(e);
      list.innerHTML =
        '<div class="text-center text-red-500 mt-10 text-sm">Ошибка загрузки подписок</div>';
    }
  } else {
    list.innerHTML =
      '<div class="text-center text-gray-500 mt-10 text-sm">У вас пока нет подписок</div>';
  }
}
...
(window as any).navigateToUserFromSubscriptions =
  navigateToUserFromSubscriptions;
...
export function performFullShortsSearch(query: string) {
  ...
  if (!query || query.trim().length === 0) {
    list.innerHTML =
      '<div class="text-center text-gray-500 mt-10 text-sm">Введите запрос для поиска</div>';
    return;
  }

  list.innerHTML =
    '<div class="text-center py-10"><div class="w-6 h-6 border-2 border-blue-500 border-t-transparent rounded-full animate-spin mx-auto"></div></div>';
    ...
    try {
      ...
      if (currentSearchFilter === "videos") {
        const { data } = await supabase
          .from("shorts")
          .select(
            "*, profiles:author_id(display_name, username, avatar_url, is_premium, premium_until)",
          )
        ...
        if (matches.length > 0) {
          ...
          list.innerHTML =
            '<div class="text-center text-gray-500 mt-10 text-sm">Ничего не найдено</div>';
        }
      } else {
        // Channels
        const { data } = await supabase
        ...
        if (data && data.length > 0) {
          ...
          list.innerHTML =
            '<div class="text-center text-gray-500 mt-10 text-sm">Каналы не найдены</div>';
        }
      }
    } catch (e) {
      console.error(e);
      list.innerHTML =
        '<div class="text-center text-red-500 mt-10 text-sm">Ошибка поиска</div>';
    }
  }, 400);
}
...
export async function viewUserShorts(userId?: string) {
  ...
  if (window.location.hash !== "#shorts-analytics") {
    window.history.pushState(
      { type: "shorts_analytics", userId: targetId },
      "",
      "#shorts-analytics",
    );
  } else {
...
async function _doViewUserShorts(targetId: string) {
  ...
  grid.innerHTML =
    '<div class="col-span-3 py-10 text-center text-gray-500"><div class="w-8 h-8 border-4 border-blue-500 border-t-transparent rounded-full animate-spin mx-auto"></div></div>';
  ...
  if (profile) {
    ...
    const premiumBadgeHtml = isPremiumUser
      ? `<div class="absolute -bottom-1 -right-1 bg-white dark:bg-gray-800 rounded-full p-0.5 shadow-sm border border-gray-200 dark:border-gray-700 z-50 w-6 h-6 flex items-center justify-center"><img src="./image/Google-Gemini-Logo-Transparent.png" class="w-full h-full object-contain" alt="Premium"></div>`
      : "";
    let avatarStr = profile.avatar_url
      ? `<img src="${profile.avatar_url}" class="w-full h-full object-cover">`
      : `<div class="w-full h-full bg-gradient-to-tr from-blue-500 to-purple-500 flex items-center justify-center text-white text-xl font-bold">${(profile.display_name || profile.username || "?")[0].toUpperCase()}</div>`;
    document.getElementById("analytics-avatar")!.innerHTML =
      `<div class="w-full h-full relative rounded-full overflow-hidden">${avatarStr}</div>${premiumBadgeHtml}`;
    document
      .getElementById("analytics-avatar")!
      .classList.remove("overflow-hidden"); // The badge needs to overflow the main container
    document.getElementById("analytics-name")!.innerText =
      profile.display_name || profile.username;

    const subContainer = document.getElementById(
      "analytics-subscribe-container",
    );
    if (subContainer) {
  ...
  const { data: userShorts } = await supabase
    .from("shorts")
    .select("*")
  ...
  document.getElementById("analytics-stats")!.innerText =
    `${userShorts?.length || 0} публикаций`;

  const isOwner = state.currentUser ? targetId === state.currentUser.id : false;

  if (userShorts && userShorts.length > 0) {
    grid.innerHTML = userShorts
      .map(
        (s) => `
    ...
    grid.innerHTML =
      '<div class="col-span-3 py-10 text-center text-gray-500">Нет публикаций</div>';
  }
}
(window as any).viewUserShorts = viewUserShorts;
...
async function fetchShortsBatch(
  initialShortId?: string,
  authorFilterId?: string,
) {
  try {
    ...
    const { data: myLikedShorts } = await supabase
      .from("short_likes")
      .select("shorts(title, description)")
      .eq("user_id", state.currentUser!.id)
      .limit(50);
        ...
        if (s) {
          const words =
            `${s.title || ""} ${s.description || ""}`
              ...
              .split(/[\s\.,?!]+/) || [];
          words.forEach((w) => {
            if (w.length > 3) preferredWords.add(w);
          });
        }
      });
    }
    ...
    if (authorFilterId) {
      ...
      allShorts.forEach((s) => {
        ...
        score +=
          Math.log10((s.likes_count || 0) * 10 + (s.views_count || 0) + 1) * 15;
...
async function loadShorts(initialShortId?: string, authorFilterId?: string) {
    ...
    if (authorFilterId === "subscriptions") {
      ...
      if (subs.length > 0) {
        const { data: subProfiles } = await supabase
          ...
          .in("id", subs);
        if (subProfiles) {
          subBar.innerHTML = subProfiles
            .map((p) => {
              const avatarStr = p.avatar_url
                ? `<img src="${p.avatar_url}" class="w-full h-full object-cover">`
                : `<div class="w-full h-full bg-gradient-to-tr from-blue-500 to-purple-500 flex items-center justify-center text-[10px] font-bold text-white">${(p.display_name || p.username || "?")[0].toUpperCase()}</div>`;
  ...
  container.innerHTML =
    '<div class="flex h-full items-center justify-center"><div class="w-10 h-10 border-4 border-blue-500 border-t-transparent rounded-full animate-spin"></div></div>';
  ...
  supabase
    ...
    .then(({ data: myLikes }) => {
      ...
      toAppend.forEach((short, i) => {
        ...
        let avatarStr = "";
        const isPremiumUser =
          short.profiles?.is_premium &&
          (!short.profiles?.premium_until ||
            new Date(short.profiles.premium_until) > new Date());
        const premiumBadgeHtml = isPremiumUser
          ? `<div class="absolute -bottom-1 -right-1 bg-black/60 backdrop-blur rounded-full p-0.5 shadow-sm border border-gray-700 z-50 w-4 h-4 flex items-center justify-center"><img src="./image/Google-Gemini-Logo-Transparent.png" class="w-full h-full object-contain" alt="Premium"></div>`
          : "";
        ...
        const bufferingIcon = element.querySelector(
          `#buffering-icon-${index}`,
        ) as HTMLElement;
    ...
    (entries) => {
      entries.forEach((entry) => {
        ...
        if (entry.isIntersecting) {
          ...
          document
            .querySelectorAll(".short-video-wrapper video")
            .forEach((v) => {
              const vidElement = v as HTMLVideoElement;
              const parentIndex = parseInt(
                vidElement
                  .closest(".short-video-wrapper")
...
async function recordView(shortId: string) {
  ...
  try {
    ...
    if (!error) {
      ...
      if (uiViews)
        uiViews.innerText = (parseInt(uiViews.innerText || "0") + 1).toString();
...
(window as any).toggleSubscription = async (
  authorId: string,
  isAnalytics = false,
) => {
  ...
  try {
    ...
    const analyticsBtn = document.getElementById(
      `subscribe-btn-analytics-${authorId}`,
    );
  ...
  try {
    if (isLiked) {
      ...
      await supabase
        ...
        .eq("user_id", state.currentUser.id);
      if (uiLikesCount)
        uiLikesCount.innerText = Math.max(0, count - 1).toString();
      if (uiLikeBtn) {
        uiLikeBtn.classList.remove("text-red-500");
        uiLikeBtn.classList.add("text-white");
        uiLikeBtn.querySelector("svg")!.setAttribute("fill", "none");
    ...
    console.error(e);
    import("./utils").then((m) =>
      m.customAlert("Ошибка при лайке: " + e.message),
    );
  }
};
...
function pauseAllVideos() {
  document
    .querySelectorAll("#shorts-container video")
...
async function loadShortComments(shortId: string) {
  ...
  try {
    const { data: comments, error } = await supabase
      .from("short_comments")
      .select(
        `*, profiles:user_id(display_name, username, avatar_url, is_premium, premium_until)`,
      )
      .eq("short_id", shortId)
    ...
    const renderComment = (c: any, isReply: boolean = false) => {
      const name =
        c.profiles?.display_name || c.profiles?.username || "Неизвестный";
      const isPremiumUser =
        c.profiles?.is_premium &&
        (!c.profiles?.premium_until ||
          new Date(c.profiles.premium_until) > new Date());
      const premiumBadgeHtml = isPremiumUser
        ? `<div class="absolute -top-1 -left-1 bg-black/60 backdrop-blur rounded-full p-0.5 shadow-sm border border-gray-700 z-50 w-3 h-3 flex items-center justify-center"><img src="./image/Google-Gemini-Logo-Transparent.png" class="w-full h-full object-contain" alt="Premium"></div>`
        : "";
      let avatarStr = "";
...
(window as any).replyToShortComment = (
  commentId: string,
  authorName: string,
) => {
...
(window as any).sendShortComment = async () => {
  ...
  try {
    ...
    const countEl = document.getElementById(
      `short-comments-count-${currentShortIdForComments}`,
    );
    if (countEl)
      countEl.innerText = (parseInt(countEl.innerText || "0") + 1).toString();

    const { data } = await supabase
      .from("shorts")
      .select("comments_count")
      .eq("id", currentShortIdForComments)
      .single();
    if (data) {
      const { error } = await supabase
        .from("shorts")
        ...
        .eq("id", currentShortIdForComments);
      if (error)
        console.error("Failed to update shorts comments_count:", error);
    }
    ...
    console.error(e);
    import("./utils").then((m) =>
      m.customAlert("Ошибка при отправке комментария: " + e.message),
    );
  }
};
...
(window as any).openUploadShort = () => {
  ...
  if (progressOverlay)
    progressOverlay.className =
      "hidden absolute inset-0 bg-gray-900/80 flex-col items-center justify-center z-20 backdrop-blur-sm";
...
(window as any).handleShortFileSelect = (event: any) => {
  ...
  if (file.type.indexOf("video") === -1) {
    import("./utils").then((m) =>
      m.customToast("Пожалуйста, выберите видео файл"),
    );
    return;
  }
  ...
  if (file.size > maxSizeMB * 1024 * 1024) {
    import("./utils").then((m) =>
      m.customToast(
        `Размер видео не должен превышать ${maxSizeMB}МБ.${!isPremium ? " Приобретите Premium для увеличения лимита до 200МБ!" : ""}`,
      ),
    );
    return;
  }
...
(window as any).submitShort = async () => {
  ...
  if (title.length > 25) {
    import("./utils").then((m) =>
      m.customToast("Название не должно превышать 25 символов"),
    );
    return;
  }
  if (desc.length > 200) {
    import("./utils").then((m) =>
      m.customToast("Описание не должно превышать 200 символов"),
    );
    return;
  }
  ...
  try {
    ...
    if (progressOverlay)
      progressOverlay.className =
        "absolute inset-0 bg-gray-900/80 flex flex-col items-center justify-center z-20 backdrop-blur-sm";
    ...
    console.error(e);
    import("./utils").then((m) =>
      m.customAlert("Ошибка загрузки: " + e.message),
    );
  } finally {
    if (progressOverlay)
      progressOverlay.className =
...
export async function deleteShort(shortId: string) {
  ...
  const { data: shortData } = await supabase
    .from("shorts")
    .select("author_id, video_url")
    .eq("id", shortId)
    .single();
  if (!shortData) return;
  const isAdmin =
    state.currentProfile?.settings?.isAdmin ||
    state.currentProfile?.settings?.is_tech_support;
  if (shortData.author_id !== state.currentUser.id && !isAdmin) return;
  ...
  const { data: delData, error: sErr } = await supabase
    .from("shorts")
    .delete()
    .eq("id", shortId)
    .select();
  ...
  if (stillExists) {
    import("./utils").then((m) =>
      m.customAlert(
        "Ошибка: Недостаточно прав для удаления (необходима политика RLS для DELETE) или есть связанные данные. Шортс скрыт только локально.",
      ),
    );
  } else {
  ...
  const el = document.querySelector(
    `.short-video-wrapper[data-id="${shortId}"]`,
  );
  if (el) el.remove();
...
window.addEventListener("popstate", (e) => {
  ...
  if (hash === "#shorts-subscriptions") {
    ...
    document
      .getElementById("shorts-subscriptions-overlay")
