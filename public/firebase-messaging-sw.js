...
self.addEventListener('notificationclick', function(event) {
    ...
    // Custom target url from our push handler (if it was created by us)
    let targetUrl = event.notification.data?.url;
    
    // If it was auto-generated by FCM / browser
    if (!targetUrl) {
        const fcmData = event.notification.data?.FCM_MSG?.data || event.notification.data || {};
        let chatId = fcmData.chat_id || fcmData.chatId;
        
        targetUrl = self.location.origin + self.location.pathname.replace('firebase-messaging-sw.js', '');
        if (chatId) {
            targetUrl += '#chat=' + chatId;
        }
    }
