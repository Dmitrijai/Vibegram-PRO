  ...
  try {
    ...
    chats.forEach((chat: any) => {
      if (chat.messages) {
        chat.messages.sort(
          (m1: any, m2: any) =>
            new Date(m1.created_at).getTime() -
    ...
    let savedMessagesIdx = chats.findIndex(
      (c: any) => c.description === "SAVED_MESSAGES" && c.type === "private",
    );
    if (savedMessagesIdx === -1) {
      // fallback for older accounts
      savedMessagesIdx = chats.findIndex(
        (c: any) =>
          c.type === "private" &&
          (!c.chat_members ||
    ...
    if (savedMessagesIdx !== -1) {
      ...
      if (savedMessagesChat.description !== "SAVED_MESSAGES") {
        ...
        supabase
          .from("chats")
          .update({ description: "SAVED_MESSAGES", title: "Избранное" })
          .eq("id", savedMessagesChat.id)
          .then();
      }
    }

    const showSavedMessages =
      state.currentProfile?.settings?.show_saved_messages !== false;

    const activeChatIsSavedMessages =
      !state.activeChatIsGroup &&
      state.activeChatMembers?.length > 0 &&
      state.activeChatMembers.every(
        (m: any) => m.user_id === state.currentUser?.id,
      );
    const isSavedActive =
      state.activeChatId === "new_saved_messages" ||
      (savedMessagesChat && savedMessagesChat.id === state.activeChatId) ||
      activeChatIsSavedMessages;
    ...
    chats.forEach((chat: any) => {
      if (
        chat.description === "POST_COMMENTS" ||
        (chat.title === "Комментарии" &&
          chat.type === "group" &&
          chat.is_public)
      )
        return;
      ...
      let isSavedMsgs =
        chat.description === "SAVED_MESSAGES" ||
        chat.id === "new_saved_messages";

      // Auto-fix incorrectly converted direct chats
      if (chat.type === "direct" && isSavedMsgs) {
        isSavedMsgs = false;
        chat.title = null;
        chat.description = null;
        supabase
          .from("chats")
          .update({ description: null, title: null })
          .eq("id", chat.id)
          .then();
      }
      ...
      if (
        (chat.type === "direct" || chat.type === "private") &&
      ...
      if (
        chat.description === "TECH_SUPPORT_CHAT" &&
            ...
            if (other?.profiles) {
              ...
              if (isTechSupport) {
                ...
                if (isOnline && other.profiles.last_seen) {
                  if (
                    new Date().getTime() -
                      new Date(other.profiles.last_seen).getTime() >
                    3 * 60 * 1000
                  ) {
                    isOnline = false;
                  }
      ...
      if (lastMsg) {
        ...
        else if (lastMsg.message_type === "video_circle") previewText = "Кружок";
        ...
        if (isSpecialText) {
             previewText = `<span class="text-blue-500">${previewText}</span>`;
        }
      ...
      if (isSavedMessages) {
        ...
        if (
          ...
          const other = chat.chat_members?.find(
            (m: any) => m.user_id !== state.currentUser.id,
          );
      ...
      const unreadCount = chat.messages
        ? chat.messages.filter(
            (m: any) => !m.is_read && m.sender_id !== state.currentUser.id,
          ).length
        : 0;
      const unreadBadge =
        unreadCount > 0
          ? `<div class="bg-blue-500 text-white text-[11px] font-bold px-1.5 py-0.5 rounded-full min-w-[20px] text-center ml-2 shrink-0">${unreadCount}</div>`
          : "";

      const isMuted = (
        state.currentProfile?.settings?.muted_chats || []
      ).includes(chat.id);
      const muteBadge = isMuted
        ? `<svg class="w-3.5 h-3.5 text-gray-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5.586 15H4a1 1 0 01-1-1v-4a1 1 0 011-1h1.586l4.707-4.707C10.923 3.663 12 4.109 12 5v14c0 .891-1.077 1.337-1.707.707L5.586 15z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2"></path></svg>`
        : "";

      const otherUserProfile = !isGroup
      ...
      const isPremiumUser =
        ...
        (!otherUserProfile.premium_until ||
          new Date(otherUserProfile.premium_until) > new Date());
      ...
      div.addEventListener(
        ...
        (e) => {
          touchTimer = setTimeout(() => {
            openChatContextMenu(chat.id, chatName || "Чат", e, isSavedMessages);
          }, 600);
        },
        { passive: true },
      );
...
export async function openChatContextMenu(
  chatId: string,
  chatName: string,
  e: MouseEvent | TouchEvent,
  isSavedMessagesFlag?: boolean,
) {
  if ((window as any).closeChatContextMenu)
    (window as any).closeChatContextMenu();

  const menu = document.createElement("div");
  menu.id = "chat-context-menu";
  menu.className =
    "fixed bg-white dark:bg-gray-800 shadow-xl rounded-xl py-2 w-56 z-[100] border border-gray-100 dark:border-gray-700 max-w-[90vw]";
...
export function closeChat() {
  ...
  if (searchBar) {
    ...
    (
      document.getElementById("chat-search-input-box") as HTMLInputElement
    ).value = "";
...
export function updateTypingStatus() {
  ...
  if (state.typingUsers.size > 0) {
    ...
    else if (first.action === "recording_video")
      actionText = "записывает видео...";
    else if (first.action === "uploading_file")
      actionText = "отправляет файл...";
    ...
    if (!state.activeChatIsGroup && state.activeChatOtherUser) {
      const isTechSupport =
        state.activeChatOtherUser.settings?.is_tech_support === true;
      if (isTechSupport) {
        ...
        const status = getStatusText(
          state.activeChatOtherUser.is_online,
          state.activeChatOtherUser.last_seen,
        );
...
export function updateChatInputUI() {
  ...
  if (!state.activeChatIsGroup && state.activeChatOtherUser) {
    ...
    isBlockedByMe = (mySettings.blocked_users || []).includes(
      state.activeChatOtherUser.id,
    );
    isBlockedByThem = (otherSettings.blocked_users || []).includes(
  ...
  if (state.activeChatType === "channel") {
    const myRole = state.activeChatMembers?.find(
      (m: any) => m.user_id === state.currentUser.id,
    )?.role;
...
export async function openChat(
  ...
  if (state.activeChatId && state.activeChatId !== chatId) {
    ...
    if (list) {
      ...
      if (isAtBottom) {
        ...
        if (anchorId) {
          state.chatScrollPositions.set(state.activeChatId, {
            type: "anchor",
            id: anchorId,
            offset: anchorOffset,
          });
  ...
  if (
    ...
    state.activeChatParentInfo.messageId !== chatId
  ) {
  ...
  const updateHeaderInfo = () => {
    const statusEl = document.getElementById("current-chat-status")!;
    const isTechSupportChat =
      state.activeChatDescription === "TECH_SUPPORT_CHAT";
    let isChattingWithSupportUser = false;

    if (!state.activeChatIsGroup) {
      state.activeChatOtherUser = state.activeChatMembers?.find(
        (m: any) => m.user_id !== state.currentUser.id,
      )?.profiles;
      if (state.activeChatOtherUser) {
        isChattingWithSupportUser =
          state.activeChatOtherUser.settings?.is_tech_support === true;
      }
    } else {
    ...
    if (!state.activeChatIsGroup) {
      ...
      const activeMembersCount = state.activeChatMembers
        ? state.activeChatMembers.filter((m: any) => m.role !== "pending")
            .length
        : 0;
  ...
  if (chatType === "direct" || chatType === "private") {
    const otherUserProfile = members?.find(
      (m: any) => m.user_id !== state.currentUser?.id,
    )?.profiles;
    isPremiumUser =
      otherUserProfile?.is_premium &&
  ...
  if (
    (state.activeChatIsGroup || state.isAdminStatus) &&
    state.activeChatMembers.length === 0
  ) {
    supabase
      .from("chat_members")
      .select(
        "user_id, role, profiles(id, username, display_name, last_seen, is_online, avatar_url, bio, settings, is_premium, premium_until)",
      )
      .eq("chat_id", chatId)
      .then(({ data, error }) => {
        if (data) {
  ...
  if (backBtn) {
    if (state.activeChatParentInfo) {
      backBtn.className =
        "mr-3 p-2.5 text-blue-500 hover:bg-blue-100 dark:hover:bg-gray-700 rounded-full flex items-center justify-center transition-colors shadow-sm bg-blue-50 dark:bg-gray-800 shrink-0";
      ...
      backBtn.className =
        "md:hidden mr-2 p-2.5 text-blue-500 hover:bg-blue-100 dark:hover:bg-gray-700 rounded-full flex items-center justify-center shrink-0";
  ...
  const isSavedMsgHeader =
    (!isGroup &&
      (!members ||
        members.filter((m: any) => m.user_id !== state.currentUser?.id)
          .length === 0)) ||
    chatName === "Избранное";
...
(window as any).performChatSearch = async () => {
  ...
  const dateInput = (
    document.getElementById("chat-search-date") as HTMLInputElement
  ).value;
