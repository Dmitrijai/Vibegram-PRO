        ...
        <div class="space-y-3 mb-6 mt-4">
            ...
            <details class="group bg-gray-50 dark:bg-gray-800 rounded-xl overflow-hidden border border-gray-100 dark:border-gray-700 open:shadow-sm transition-all ${mode === 'profile' ? 'hidden' : ''}">
                ...
                <div class="border-t border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800/80 p-2 space-y-1">
                    <button onclick="import('./utils').then(m => m.closeModal(undefined, true)); window.history.replaceState({ screen: 'create' }, '', '#create-channel'); window.openCreateChannel(true);" class="w-full text-left p-3 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition flex items-center gap-3 text-gray-700 dark:text-gray-200">
                        <svg class="w-5 h-5 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5.882V19.24a1.76 1.76 0 01-3.417.592l-2.147-6.15M18 13a3 3 0 100-6M5.436 13.683A4.001 4.001 0 017 6h1.832c4.1 0 7.625-1.234 9.168-3v14c-1.543-1.766-5.067-3-9.168-3H7a3.988 3.988 0 01-1.564-.317z"></path></svg>
                        Создать канал
                    </button>
                    <button onclick="import('./utils').then(m => m.closeModal(undefined, true)); window.history.replaceState({ screen: 'create' }, '', '#create-group'); window.openCreateGroup(true);" class="w-full text-left p-3 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition flex items-center gap-3 text-gray-700 dark:text-gray-200">
                    ...
                    <button onclick="import('./utils').then(m => m.closeModal(undefined, true)); window.history.replaceState({ type: 'shorts' }, '', '#shorts'); window.openShorts(undefined, undefined, true);" class="w-full text-left p-3 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition flex items-center gap-3 text-gray-700 dark:text-gray-200">
                        <svg class="w-5 h-5 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                        Shorts
                    </button>
                    <button onclick="import('./utils').then(m => m.closeModal(undefined, true)); window.history.replaceState({}, '', '#miniapps'); window.openMiniAppsCatalog(true);" class="w-full text-left p-3 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition flex items-center gap-3 text-gray-700 dark:text-gray-200">
