...
function renderContent(content: string) {
    ...
    safeContent = safeContent.replace(urlRegex, (url) => {
        return `<a href="${url}" target="_blank" rel="noopener noreferrer" class="text-blue-500 hover:underline break-all active:scale-95 active:opacity-70 transition-all duration-200 origin-left inline-block" onclick="event.stopPropagation()">${url}</a>`;
    });
...
export async function loadMessages(chatId: string, isInitialLoad = false) {
    ...
    try {
        const { data: messages, error } = await supabase.from('messages')
            .select('*, profiles(username, display_name, is_premium, premium_until, avatar_url)')
            .eq('chat_id', chatId)
...
export function renderMessages(messages: any[], isInitialLoad = false) {
    ...
    messages.forEach((msg, i) => {
        ...
        const actualMedia = mediaArr.filter((m: any) => m.type !== 'reply' && m.type !== 'forward' && m.type !== 'admin_mode' && m.type !== 'comments_enabled' && m.type !== 'share_app_content');
        ...
        if (!previewContentString.trim()) {
            if (msg.message_type === 'voice') previewContentString = '🎤 Голосовое сообщение';
            ...
            else if (msg.message_type === 'video') previewContentString = '🎥 Видео';
