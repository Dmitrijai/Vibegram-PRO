...
export function setupMiniApps() {
  ...
  supabase
    .channel("public:mini_apps")
    .on(
      "postgres_changes",
      { event: "*", schema: "public", table: "mini_apps" },
      (payload) => {
...
export function switchMiniAppTab(tab: "public" | "favorites" | "my") {
  ...
  document
    .getElementById("tab-public-apps")!
    .classList.remove("border-blue-500", "text-blue-600", "dark:text-blue-400");
  document
    .getElementById("tab-public-apps")!
...
export function closeAuthorMode() {
  ...
  const searchContainer = document.getElementById("miniapps-search-container");
  if (searchContainer && activeTab !== "my")
    searchContainer.classList.remove("hidden");
...
export async function toggleLikeMiniApp(appId: string) {
  ...
  try {
    if (isLiked) {
      ...
      const { error } = await supabase
        .from("mini_apps_likes")
        .insert({ app_id: appId, user_id: state.currentUser.id });
      if (error) throw error;
    }
  } catch (e: any) {
...
async function updateAuthorTotalStats() {
  ...
  try {
    const { data } = await supabase
      ...
      .eq("creator_id", currentAuthorFilter)
    ...
    if (currentAuthorFilter) {
      ...
      if (
        ...
        const { data: creators } = await supabase
          ...
          .select("id")
          .or(
            `username.ilike.%${currentSearchQuery}%,display_name.ilike.%${currentSearchQuery}%`,
          );
        const creatorIds = creators?.map((c) => c.id) || [];
        if (creatorIds.length > 0) {
          query = query.or(
            `title.ilike.%${currentSearchQuery}%,description.ilike.%${currentSearchQuery}%,creator_id.in.(${creatorIds.join(",")})`,
          );
        } else {
          query = query.or(
            `title.ilike.%${currentSearchQuery}%,description.ilike.%${currentSearchQuery}%`,
          );
        }
      }
    }
    ...
    if (!isDirectLinkSearch && !currentAuthorFilter) {
      if (tab === "public") {
        ...
        if (!state.currentUser) {
          listEl.innerHTML =
            '<div class="text-center p-4 text-gray-500">Войдите, чтобы просматривать избранное</div>';
          return;
        }
...
export function showMiniAppDetails(id: string) {
  ...
  document.getElementById("miniapp-details-desc")!.textContent =
...
export function openCreateMiniApp() {
  ...
  (
    document.getElementById("miniapp-public-toggle") as HTMLInputElement
  ).checked = false;
...
export async function submitMiniApp() {
  ...
  const descBtn = document.getElementById(
    "miniapp-desc",
  ...
  if (titleBtn.value.trim().length > 25)
    return customToast("Название не должно превышать 25 символов");
  if (descBtn.value.trim().length > 200)
    return customToast("Описание не должно превышать 200 символов");
  ...
  if (isFileMode) {
    if (!fileInput.files || fileInput.files.length === 0)
      return customToast("Выберите HTML файл");
    if (fileInput.files[0].size > 2 * 1024 * 1024)
      return customToast("Файл слишком большой (> 2 МБ)");
  } else {
  ...
  if (iconInput.files && iconInput.files.length > 0) {
    ...
    if (iconFile.size > 2 * 1024 * 1024)
      return customToast("Иконка слишком большая (> 2 МБ)");
  }
...
export function openEditMiniApp(
  id: string,
  title: string,
  desc: string,
  isPublic: boolean,
  ...
  if (modal) {
    ...
    (document.getElementById("edit-miniapp-title") as HTMLInputElement).value =
      title || "";
    (
      document.getElementById("edit-miniapp-desc") as HTMLTextAreaElement
    ).value = desc || "";
    (
      document.getElementById("edit-miniapp-public-toggle") as HTMLInputElement
    ).checked = isPublic;
...
export async function submitEditMiniApp() {
  ...
  const titleBtn = document.getElementById(
    "edit-miniapp-title",
  ) as HTMLInputElement;
  const descBtn = document.getElementById(
    "edit-miniapp-desc",
...
export async function deleteMiniApp(id: string) {
  ...
  try {
    ...
    if (
      ...
      appData.html_content.startsWith("https://")
    ) {
      import("./utils").then((m) =>
        m.softDeleteCloudinaryFile(appData.html_content),
      );
    } else if (appData && appData.html_url) {
      import("./utils").then((m) =>
        m.softDeleteCloudinaryFile(appData.html_url),
      );
    }
