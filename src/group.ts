...
function updateMediaSelectionUI() {
  ...
  if (!bar) {
    ...
    bar.className =
      "absolute bottom-0 left-0 right-0 bg-white/95 dark:bg-gray-800/95 backdrop-blur-md border-t border-gray-200 dark:border-gray-700 p-3 flex items-center justify-between z-50 transform transition-transform duration-300 translate-y-full rounded-b-3xl";
    bar.innerHTML = `
...
export async function deleteSelectedMedia() {
  ...
  const confirmed = await customConfirm(
    `Удалить ${selectedMedia.size} сообщений?`,
  );
  if (confirmed) {
    const ids = Array.from(selectedMedia);
    try {
...
export function startMemberLongPress(
  e: Event,
  userId: string,
  role: string,
  isCreator: boolean,
...
export function showMemberManageMenu(
  e: Event,
  userId: string,
  role: string,
  isCreator: boolean,
  ...
  menu.className =
    "fixed bg-white dark:bg-gray-800 rounded-xl shadow-2xl border border-gray-200 dark:border-gray-700 py-2 z-[9999] min-w-[200px] modal-enter overflow-hidden";
  ...
  const closeMenu = (ev: Event) => {
    if (!(ev.target as HTMLElement).closest("#member-context-menu")) {
      ...
      if (modalContent) {
        ...
        modalContent.removeEventListener("touchstart", closeMenu, {
          capture: true,
        });
...
export function startMediaLongPress(
  e: Event,
  msgId: string,
  url: string,
  name: string,
...
export function showMediaContextMenu(
  ...
  menu.className =
    "fixed bg-white dark:bg-gray-800 rounded-xl shadow-2xl border border-gray-200 dark:border-gray-700 py-2 z-[100] min-w-[160px] modal-enter";
...
export async function openChatInfo(skipPushState = false) {
  ...
  const isSavedMessages =
    state.activeChatId === "new_saved_messages" ||
  ...
  const isCommentChat =
    state.activeChatParentInfo ||
    state.activeChatDescription === "Обсуждение поста" ||
    state.activeChatDescription === "POST_COMMENTS";
  ...
  if (isSavedMessages) {
    ...
    const canManage =
      myRole === "creator" || myRole === "admin" || state.isAdminStatus;
    ...
    infoHtml = `
            <div class="mt-6 w-full">
                ...
                ${
                  isChannel && !canManage
                    ? '<div class="text-xs text-gray-500 mb-4 px-2">Список подписчиков скрыт администраторами.</div>'
                    : `
    ...
    infoHtml = `
            <div class="mt-6 w-full bg-gray-50 dark:bg-gray-800 p-4 rounded-2xl flex flex-col items-center text-center">
                <div class="w-16 h-16 bg-gray-200 dark:bg-gray-700 rounded-full flex items-center justify-center text-gray-400 mb-3">
                    <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
                </div>
                <div class="text-sm text-gray-500 mb-2">Аккаунт пользователя был удален.</div>
            </div>
            <div class="mt-4 w-full space-y-2">
                <button onclick="clearHistory()" class="w-full text-left px-4 py-3 text-sm text-orange-500 hover:bg-orange-50 dark:hover:bg-orange-900/30 rounded-xl transition-colors flex items-center gap-3 font-medium">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
...
export async function saveGroupSettings() {
  const desc = (
    document.getElementById("group-description-input") as HTMLTextAreaElement
  ).value.trim();
  const isPublicToggle = document.getElementById(
    "channel-public-toggle",
  ) as HTMLInputElement;
  const isPublic = isPublicToggle ? isPublicToggle.checked : false;

  await supabase
    .from("chats")
    .update({ description: desc, is_public: isPublic })
    .eq("id", state.activeChatId);
...
export async function toggleBlockUser(userId: string) {
  ...
  if (blocked_users.includes(userId)) {
    settings.blocked_users = blocked_users.filter(
      (id: string) => id !== userId,
    );
  } else {
...
export async function rejectJoinRequest(
  ...
  // Update local state
  state.activeChatMembers = state.activeChatMembers.filter(
...
export async function kickMember(userId: string) {
  const confirmed = await customConfirm(
    "Удалить этого пользователя из группы?",
  );
  if (!confirmed) return;
  await supabase
    .from("chat_members")
...
export async function toggleMuteChat() {
  ...
  if (mutedChats.includes(state.activeChatId)) {
    settings.muted_chats = mutedChats.filter(
      (id: string) => id !== state.activeChatId,
    );
  } else {
...
export async function searchUsersForAdding(query: string) {
  ...
  const { data, error } = await supabase
    .from("profiles")
    .select(
      "id, username, display_name, avatar_url, settings, is_premium, premium_until",
    )
  ...
  if (error || !data || data.length === 0) {
    resultsContainer.innerHTML =
      '<div class="p-4 text-center text-gray-500 dark:text-gray-400 text-sm">Ничего не найдено</div>';
  ...
  const filteredData = data
    .filter(
      (u) =>
        !(u.settings && u.settings.is_tech_support) &&
        !currentMemberIds.includes(u.id) &&
        !state.groupCreationSelectedUsers.find((su) => su.id === u.id),
    )
    .slice(0, 10);

  if (filteredData.length === 0) {
    resultsContainer.innerHTML =
      '<div class="p-4 text-center text-gray-500 dark:text-gray-400 text-sm">Все найденные пользователи уже в группе</div>';
    resultsContainer.classList.remove("hidden");
    return;
  }
...
export function removeUserFromAdding(id: string) {
  state.groupCreationSelectedUsers = state.groupCreationSelectedUsers.filter(
    (u) => u.id !== id,
  );
  renderAddMemberSelectedUsers();
}
...
export function renderAddMemberSelectedUsers() {
  ...
  container.innerHTML = state.groupCreationSelectedUsers
    .map(
      (u) => `
...
export function openCreateGroup(skipPushState = false) {
  if (state.currentProfile?.settings?.is_tech_support) {
    import("./utils").then((m) =>
      m.customAlert("Техническая поддержка не может создавать группы."),
    );
    return;
  }
...
export function openCreateChannel(skipPushState = false) {
  if (state.currentProfile?.settings?.is_tech_support) {
    import("./utils").then((m) =>
      m.customAlert("Техническая поддержка не может создавать каналы."),
    );
    return;
  }
...
export function renderCreateGroupModal(type: "group" | "channel" = "group") {
  const currentNameInput = document.getElementById(
    "group-name",
  ) as HTMLInputElement;
  if (currentNameInput) {
    (window as any).tempGroupName = currentNameInput.value;
  }
  const currentUsernameInput = document.getElementById(
    "group-username",
  ) as HTMLInputElement;
...
export async function searchGroupUsers(
  q: string,
  ...
  gSearchTimeout = setTimeout(async () => {
    const { data: rawData } = await supabase
      ...
      .limit(30);
    const data = rawData
      ?.filter((u: any) => !(u.settings && u.settings.is_tech_support))
      ?.slice(0, 5);
    ...
    if (data && data.length > 0) {
      data.forEach((u) => {
        ...
        const premiumBadgeHtml = isPremiumUser
          ? `<div class="absolute -top-1 -left-1 bg-white dark:bg-gray-800 rounded-full p-0.5 shadow-sm border border-gray-200 dark:border-gray-700 z-50 w-3 h-3 flex items-center justify-center"><img src="./image/Google-Gemini-Logo-Transparent.png" class="w-full h-full object-contain" alt="Premium"></div>`
          : "";
        if (state.groupCreationSelectedUsers.find((su) => su.id === u.id))
          return;
      ...
      resultsBox.innerHTML =
        '<div class="p-4 text-center text-gray-500 text-sm">Ничего не найдено</div>';
...
export async function createGroup() {
  ...
  if (name.length < 3 || name.length > 35)
    return customAlert("Название должно быть от 3 до 35 символов");

  const username = (
    document.getElementById("group-username") as HTMLInputElement
  ).value.trim();
  if (!username || username.length < 3)
    return customAlert("Введите корректный ID (от 3 символов)");
  ...
  if (existingChats && existingChats.length > 0)
    return customAlert("Данный ID уже занят");

  const isPublic =
    (document.getElementById("chat-is-public") as HTMLInputElement)?.checked ||
    false;
  ...
  const members = [
    { chat_id: newChatId, user_id: state.currentUser.id, role: "creator" },
  ];
  state.groupCreationSelectedUsers.forEach((u) =>
    members.push({ chat_id: newChatId, user_id: u.id, role: "member" }),
  );
  await supabase.from("chat_members").insert(members);

  window.history.replaceState(
    { screen: "chat", chatId: newChatId },
    "",
    "#chat",
  );
...
export async function leaveGroup() {
  ...
  if (myMember?.role === "creator") {
    const confirmed = await customConfirm(
      "Вы создатель группы. Если вы покинете её, группа будет удалена для всех участников. Продолжить?",
    );
    if (!confirmed) return;
...
export async function clearHistory() {
  const confirmed = await customConfirm(
    "Вы уверены, что хотите очистить историю сообщений? Это действие нельзя отменить.",
  );
  if (!confirmed) return;
...
export async function checkGlobalPendingRequests() {
  ...
  const { data: pendingMembers } = await supabase
    .from("chat_members")
    .select("*, profiles(*), chats(title)")
    .in("chat_id", adminChatIds)
    .eq("role", "pending");
...
export function showGlobalPendingModal(pendingMembers: any[]) {
  ...
  if (!modal) {
    ...
    modal.className =
      "fixed inset-0 bg-black/50 z-[1000] flex items-center justify-center p-4 backdrop-blur-sm transition-opacity";
...
(window as any).handleGlobalJoinRequest = async (
  userId: string,
  chatId: string,
